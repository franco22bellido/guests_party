...
export class EventsController {
  ...
  @UseGuards(AuthGuard)
  @Get('')
  findAll(@Req() requestUser: RequestUser) {
    return this.eventsService.findAll(requestUser.user.id);
  }

  @UseGuards(AuthGuard)
  @Get(':eventId/')
  findOne(@Param('eventId') eventId: number, @Req() requestUser: RequestUser) {
  ...
  @UseGuards(AuthGuard)
  @Delete(':id')
  remove(@Param('id') id: number, @Req() requestUser: RequestUser) {
    return this.eventsService.delete(requestUser.user.id, id);
  }

  @UseGuards(AuthGuard)
  @Get('EventAndGuests/:id')
  getGuests(@Param('id') id: number, @Req() requestUser: RequestUser) {
